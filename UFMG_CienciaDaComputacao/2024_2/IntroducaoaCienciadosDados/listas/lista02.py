...
Original file is located at
    https://colab.research.google.com/drive/1wGafuYVAnL1td6lT9G-0OBN6rlbPxkwu
...
ax = df.plot(kind='bar', figsize=(12, 8), edgecolor='black')
...
def north_or_south(df):
    # retorne 'Norte' ou 'Sul'
    avg_icecream = df['icecream'].mean()
    avg_sunglasses = df['sunglasses'].mean()
    avg_coats = df['coats'].mean()

    # Se a média de vendas de casacos for maior que a de óculos de sol e sorvetes, está no hemisfério norte
    if avg_coats > avg_icecream and avg_coats > avg_sunglasses:
        return 'Norte'
    else:
        return 'Sul'
...
def mortes_por_pump(df):
    mortes_por_bomba = df.groupby('NearestPumpID')['Count'].sum()
    return mortes_por_bomba